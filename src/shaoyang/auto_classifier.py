...
def classify_by_vectors(embeddings: np.ndarray, texts: List[str], file_names: List[str], top_k: int = 3) -> List[Dict]:
    """
    对每个 chunk 进行语义分类
    Args:
...
def reclassify_store(store, batch_size: int = 200) -> Tuple[int, int]:
    ...
    # 获取所有需重新分类的 chunk
    rows = conn.execute("SELECT rowid, text, file_name, category FROM chunks WHERE status='active'").fetchall()
    ...
    for i in range(0, total, batch_size):
        ...
        for j, r in enumerate(batch):
            ...
            if best_score > 0 and best_cat != r["category"]:
                try:
                    conn.execute("UPDATE chunks SET category=? WHERE rowid=?", (best_cat, r["rowid"]))
                    updated += 1
...
def sync_category_graph(category_counts: Dict[str, int]):
    ...
    try:
        ...
        for cat, count in category_counts.items():
            graph.upsert_entity(
                name=cat, entity_type="category", metadata={"chunk_count": count, "auto_classified": True}
            )
        graph.save()
