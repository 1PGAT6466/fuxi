...
from src.config import DEEPSEEK_BASE_URL as DEEPSEEK_BASE
from src.core import clean_text, get_deepseek_key, make_id, now_iso, parse_json
...
ECOLOGY_SUB = {
    "组织权限中心": {
        "file": ["组织权限", "权限中心"],
        "content": ["行政区域", "岗位设置", "人员卡片", "账户中心", "角色设置", "矩阵设置"],
    },
    "流程引擎": {
        "file": ["流程引擎", "workflow", "流程"],
        "content": ["表单管理", "字段管理", "路径管理", "流程节点", "表单设计器"],
    },
...
DEEP_FALLBACK = {
    "02-机械设计 > 标准件库": {
        "patterns": [
            r"(GUC|PRJ|Tool\s*Spec|标准插针头|加工件|库存优先选用|模组|伺服电机|MISUMI|C-FL|C-SB|C-LM|滚珠轴承|BOM|标准件|紧固件|轴承|线轨)"
        ],
...
def _classify_weaver_ecology(fname: str, sample: str) -> dict | None:
    ...
    is_weaver = any(kw.lower() in sample[:300] for kw in WEAVER_KW) or any(
        kw in fname
        for kw in [
            "泛微",
            "E-cology",
...
def _entity_type(name: str, ctx: str = "") -> str:
    nu = name.upper()
    if re.match(r"^(LSW|AR|AP|AC|FW)\d+$", nu):
        return "network_device"
    if re.match(r"^[A-Z]{2,6}[- ][A-Z0-9]{2,12}", nu):
        return "standard_part"
    if any(s in name for s in ["MISUMI", "Mitsubishi", "SMC", "FESTO", "OMRON", "Keyence", "西门子", "三菱", "基恩士"]):
        return "supplier"
    if re.match(r"^(SUJ2|SKD|S136|DC53|NAK80|718H|P20|H13|LCP|PA66|PBT|POM|PPS|PA6|PA12|ABS|PC|PP|PE|PEEK)$", nu):
        return "material"
    if any(kw in ctx for kw in ["手册", "指南", "操作", "教程", "安装", "配置"]):
...
文档:\n{docs[:6000]}"""
    ...
    return {
        ...
        "terms": all_terms,
        "relations": [],
        "cross_links": {"wiki_entity_pairs": cross_we, "wiki_term_pairs": cross_wt},
    }
    ...
    for path, result in results:
        for wp in result.get("wiki_pages", []):
            ...
            if ex:
                continue
            conn.execute(
                "INSERT INTO wiki_pages (id,title,summary,content,category_path,quality_score,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)",
        ...
        if entity_rows:
            conn.executemany(
                "INSERT OR IGNORE INTO entities (id,name,type,category_path,created_at) VALUES (?,?,?,?,?)", entity_rows
            )
        ...
        if rel_rows:
            conn.executemany(
                "INSERT OR IGNORE INTO entity_relations (from_id,to_id,relation_type) VALUES (?,?,?)", rel_rows
            )
        ...
        if term_rows:
            conn.executemany(
                "INSERT OR IGNORE INTO terms (id,term,definition,category,created_at) VALUES (?,?,?,?,?)", term_rows
            )
        ...
        for wn, en in cl.get("wiki_entity_pairs", []):
            wid = make_id("wiki", f"{wn}{path}")
            eid = emap.get(en)
            if wid and eid:
                we_rows.append((wid, eid))
        for wn, tn in cl.get("wiki_term_pairs", []):
            wid = make_id("wiki", f"{wn}{path}")
            tid = tmap.get(tn)
            if wid and tid:
                wt_rows.append((wid, tid))
        if we_rows:
...
async def run_full_async(incremental: bool = True) -> dict:
    ...
    async with aiohttp.ClientSession() as session:
        await asyncio.gather(*[bounded_distill(session, path, batch) for path, batch in tasks])
    ...
    logger.info(f"Distill complete: {len(tasks)} batches, " f"{len(distilled)} chunks distilled")
