...
class RedisStreamTaskQueue:
    ...
    async def _update_task_status(
        self, task_id: str, status: TaskStatus, result: Optional[Dict[str, Any]] = None, error: Optional[str] = None
    ):
        """更新任务状态"""
        task_key = f"task:status:{task_id}"
        ...
        while self._running:
            try:
                ...
                messages = await self.redis.xreadgroup(
                    self.group_name, self.consumer_name, {self.stream_name: ">"}, count=10, block=5000  # 5秒超时
                )
...
async def get_task_queue() -> RedisStreamTaskQueue:
    ...
    if _task_queue is None:
        # v1.50 修复: 记录调用栈但不抛异常，返回 None
        import traceback

        stack = "".join(traceback.format_stack())
        logger.warning(f"[task_queue] get_task_queue() 被调用但未初始化，调用栈:\n{stack}")
