...
async def verify():
    ...
    logger.info("✅ 任务 1: add_event/get_event_count 正常")
    ...
    logger.info("✅ 任务 1: add_entity/get_entity_count 正常")

    # 3. 验证 events 查询
    events = store.get_all_events()
    assert len(events) == 1
    logger.info(f"✅ 任务 1: get_all_events 返回 {len(events)} 条")
    events_by_chunk = store.get_events_by_chunk_id("chunk_a_0")
    assert len(events_by_chunk) == 1
    logger.info("✅ 任务 1: get_events_by_chunk_id 正常")

    # 4. 验证 entities 查询
    entities = store.get_all_entities()
    assert len(entities) == 1
    logger.info(f"✅ 任务 1: get_all_entities 返回 {len(entities)} 条")
    entities_by_name = store.get_entities_by_name("张三")
    assert len(entities_by_name) == 1
    logger.info("✅ 任务 1: get_entities_by_name 正常")
