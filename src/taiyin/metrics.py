...
# ── 请求级指标 ──
http_requests_total = Counter("fuxi_http_requests_total", "HTTP 请求总数", ["method", "endpoint", "status"])
http_request_duration_seconds = Histogram(
    "fuxi_http_request_duration_seconds",
    "HTTP 请求延迟",
    ["method", "endpoint"],
...
# ── 搜索指标 ──
search_requests_total = Counter("fuxi_search_requests_total", "搜索请求总数", ["status"])  # success / empty / error
search_duration_seconds = Histogram(
    "fuxi_search_duration_seconds", "搜索延迟", buckets=[0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 30.0, 60.0]
)
search_result_count = Histogram("fuxi_search_result_count", "搜索结果数", buckets=[0, 1, 3, 5, 10, 20, 50])
...
# ── Token 消耗 ──
token_consumed_total = Counter("fuxi_token_consumed_total", "Token 消耗总数", ["model", "type"])  # input / output
token_cost_total = Counter("fuxi_token_cost_total", "Token 累计费用(USD)", ["model"])

# ── LLM 调用指标 ──
llm_requests_total = Counter("fuxi_llm_requests_total", "LLM 调用总数", ["model", "status"])
llm_duration_seconds = Histogram(
    "fuxi_llm_duration_seconds", "LLM 调用延迟", ["model"], buckets=[0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 30.0]
)

# ── 反馈指标 ──
feedback_total = Counter("fuxi_feedback_total", "用户反馈总数", ["action"])  # useful / useless / click / copy
