...
# javascript: 协议拦截
_JS_PROTOCOL_PATTERN = re.compile(r"(?:javascript|vbscript|livescript|data)\s*:", re.IGNORECASE)
...
_DATA_URI_PATTERN = re.compile(
    r"data\s*:\s*(?:text/html|application/javascript|text/javascript|image/svg\+xml)\s*[;,]", re.IGNORECASE
)
...
def sanitize_user_input(query: str) -> Optional[str]:
    ...
    try:
        from src.infra import detect_injection
