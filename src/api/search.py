...
async def _search_impl(q: str, top_k: int, page: int, page_size: int, granularity: str, request: Request = None):
    ...
    try:
        from src.taiyang.retrieval import event_search, hybrid_search
        ...
        if granularity == "event":
            ...
            # Chunk 粒度（默认）/ auto
            logger.info(f"[Search] q={q!r}, top_k={top_k}, granularity={granularity}, tenant_id={tenant_id}")
            results = await hybrid_search(q, top_k=top_k, granularity=granularity, tenant_id=tenant_id)
            logger.info(f"[Search] hybrid_search returned {len(results)} results")
    ...
    try:
        ...
        try:
            ...
            logs = [
                ...
                if entry.get("path", "") in search_paths and entry.get("details", {}).get("query")
            ]
        except ImportError:
            pass
        except (ImportError, ModuleNotFoundError, ValueError, KeyError) as e:
