...
# 2. POST /api/auth/login (有效输入)
print("\n" + "=" * 60)
print("2. POST /api/auth/login (有效输入)")
...
# 3. POST /api/auth/login (无效输入)
print("\n" + "=" * 60)
print("3. POST /api/auth/login (无效输入)")
test_endpoint("登录-无效密码", "POST", "/api/auth/login", 
    data={"username": "admin", "password": "wrongpassword"}, expected_status=401)

# 4. POST /api/auth/register (有效输入)
print("\n" + "=" * 60)
print("4. POST /api/auth/register (有效输入)")
test_endpoint("注册-有效输入", "POST", "/api/auth/register", 
    data={"username": "testuser", "password": "testpass123", "email": "test@example.com"})

# 5. POST /api/auth/register (无效输入)
print("\n" + "=" * 60)
print("5. POST /api/auth/register (无效输入)")
...
test_endpoint("搜索", "GET", "/api/search?q=test")
