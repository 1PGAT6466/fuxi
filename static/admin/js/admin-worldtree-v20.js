...
async function adminFetchRemoteHashes() {
  try {
    var r = await fetch('http://172.25.30.16:8090/api/files');
    var d = await r.json();
    _adminRemoteHashes = {};
...
async function adminProcessUpload() {
  ...
  for (var i = 0; i < total; i++) {
    ...
    try {
      var resp;
    try {
      resp = await fetch('http://172.25.30.16:8090/api/upload', { method: 'POST', body: fd, signal: _adminUploadAbortController.signal });
    } catch(fetchErr) {
