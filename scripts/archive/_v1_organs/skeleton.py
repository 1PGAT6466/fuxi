# 注意：本文件 (skeleton.py v1.50) 比 organs/skeleton/signal_layer.py (v1.43) 更新。
# 运行时通过 organs/skeleton/__init__.py 实际生效的是 signal_layer.py（旧版）。
# 本文件 (v1.50) 已继承 OrganBase + 易学元数据，而 signal_layer.py (v1.43) 仍为独立类。
# 建议在 v1.51 将本文件的改动合并回 signal_layer.py，然后删除本文件。
"""
organs/skeleton.py — 🦴 骨骼（知识图谱）v1.50

骨骼 = 伏羲的身体骨架，撑起整个知识结构。
实体关系推理，支撑大脑的多步推导。
v1.50: 继承 OrganBase + 易学元数据（艮 ☶ 土 戊己 西北）
