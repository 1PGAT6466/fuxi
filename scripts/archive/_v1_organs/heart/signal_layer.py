...
# ========================================================================
# deprecated: 功能已迁移到 bagua（伏羲 v2.1 Phase 1 器官→八卦迁移）
#   - 健康检查/异常告警  → src.bagua.qian.QianGua (check_health, start_beating)
#   - 短期记忆(200条)    → src.bagua.kun.KunGua (store_conversation, recall_conversation)
#   - 用户偏好          → src.bagua.kun.KunGua (set_preference, get_preference)
# 请使用 bagua 模块的 QianGua / KunGua 类代替 HeartAgent。
# ========================================================================
...
class HeartAgent(OrganBase):
    ...
    def _heal(self, anomalies: list) -> None:
    ...
    def _handle_store_memory(self, signal: Signal) -> None:
        """v1.43: 存储对话到短期记忆"""
    ...
    def _handle_recall(self, signal: Signal) -> None:
        """v1.43: 回忆最近的 N 条对话"""
        n = signal.payload.get("n", 10)
        self.meridian.reply(signal, {"history": self._short_term[-n:]})

    def _handle_user_preference(self, signal: Signal) -> None:
        """v1.43: 存储或读取用户偏好"""
