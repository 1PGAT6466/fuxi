# DEPRECATED: 本文件已被 organs/nose/signal_layer.py 取代，保留仅为向后兼容。
# 实际生效代码见 organs/nose/signal_layer.py
...
class NoseAgent(OrganBase):
    ...
    async def _check_search_logs(self) -> List[Dict]:
        alerts = []
        try:
    ...
    async def _check_zero_results(self) -> List[Dict]:
        """检查持续零结果趋势"""
        alerts = []
        try:
            from src.db.data_store import search_history
    ...
    async def _check_latency(self) -> List[Dict]:
        """检查响应延迟趋势"""
        alerts = []
        try:
            from src.db.data_store import search_history
    ...
    async def stop_sniffing(self) -> None:
        self._running = False
        if self._task:
            self._task.cancel()
