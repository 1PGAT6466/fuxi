...
#   - Wiki接收→存储→推送  → src.bagua.kun.KunGua (push_to_wiki, recall_wiki, get_wiki_stats)
# 请使用 bagua 模块的 KunGua 类代替 SpleenAgent。
# ========================================================================
...
class SpleenAgent(OrganBase):
    ...
    def _auto_refine_wiki(self, chunks: list, file_path: str) -> int:
    ...
    def _classify_text(self, text: str) -> str:
        ...
        try:
            ...
            logger.debug("[suppressed] return result", exc_info=True)
            pass
        t = text.lower()
    ...
    def _handle_data_purged(self, signal: Signal) -> None:
        """肾清除了废物 → 同步 Wiki 向量索引"""
    ...
    def _search_wiki(self, query: str, top_k: int = 3) -> List[Dict]:
        """搜索 Wiki —— 走统一 wiki 引擎"""
        try:
            from src.services.wiki import get_wiki_engine
            we = get_wiki_engine()
            ...
            if hasattr(we, '_wiki_collection') and we._wiki_collection:
                try:
                    ...
                    logger.error("[suppressed] return we.vector_search_wiki(e", exc_info=True)
                    pass
