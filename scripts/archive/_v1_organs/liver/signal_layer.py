...
#   - 免疫记忆(黑名单)    → src.bagua.kan.KanGua (filter_by_immune_memory, learn_harmful_source)
#   - 发烧检测(阈值告警)  → src.bagua.gen.GenGua (detect_fever — 通过 detect_zero_results 等嗅探方法)
# 请使用 bagua 模块的 KanGua / GenGua 类代替 LiverAgent。
# ========================================================================
...
class LiverAgent(OrganBase):
    ...
    def _handle_learn(self, signal: Signal) -> None:
    ...
    def _filter(self, results: List[Dict], query: str) -> List[Dict]:
        clean = []
        for r in results:
    ...
    def _handle_detect_fever(self, signal: Signal) -> None:
