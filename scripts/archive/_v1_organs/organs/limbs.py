# 注意：本文件 (limbs.py v1.50) 比 organs/limbs/signal_layer.py (v1.41) 更新。
# 运行时通过 organs/limbs/__init__.py 实际生效的是 signal_layer.py（旧版）。
# 本文件包含 v1.50 新增的 start_heartbeat/stop_heartbeat 心跳事件循环。
# 建议在 v1.51 将本文件的改动合并回 signal_layer.py，然后删除本文件。
"""
organs/limbs.py — 💪 四肢（执行能力）v1.50

四肢 = 伏羲的执行器官。
左手检索、右手查表、口表达。
v1.50: 添加自主心跳事件循环 (start_heartbeat / stop_heartbeat)
    ...
    async def _search(self, query: str, top_k: int = 10, category: str = "") -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                logger.warning("(ImportError, AttributeError) 失败: %s", e, exc_info=True)
    ...
    async def _heartbeat_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.error(f"[Limbs] Heartbeat loop error: {e}")
